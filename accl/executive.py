...
def calculate_budget_values(sales_df, budget_df, selected_month, sales_executives,
    ...
    try:
        ...
        month_sales['Executive_Clean'] = month_sales[sales_exec_col].astype(str).str.strip().str.upper()
        month_sales['SL_Code_Clean'] = month_sales[sales_sl_code_col].astype(str).str.strip().str.upper()
        
        budget_df['Executive_Clean'] = budget_df[budget_exec_col].astype(str).str.strip().str.upper()
        ...
        if selected_branches:
            # Apply area filtering to both datasets
            if sales_area_col in month_sales.columns:
                month_sales[sales_area_col] = month_sales[sales_area_col].apply(extract_area_name).astype(str).str.strip().str.upper()
        ...
        print(f"👥 Executives in sales: {len(sales_executives_list)}")
        print(f"👥 Executives in budget: {len(budget_executives_list)}")
        print(f"👥 All executives: {all_executives}")
        
        # Filter by selected executives if provided
        if sales_executives:
            selected_clean = [str(exec).strip().upper() for exec in sales_executives]
        ...
        # Add totals
        def add_total_row(df, qty_label, value_label=None):
            if value_label:
                budget_col = f'Budget {value_label}'
                billed_col = f'Billed {value_label}'
            else:
        ...
        result_value = add_total_row(result_value, 'Value', 'Value')
