...
def calculate_budget_values(sales_df, budget_df, selected_month, sales_executives,
    ...
    try:
        ...
        sales_executives = [str(exec).strip().upper() for exec in sales_executives]

        # Log executive names for debugging
        budget_executives = budget_df[budget_exec_col].dropna().unique().tolist()
        logger.info(f"Sales executives: {sales_executives}")
        logger.info(f"Budget executives: {budget_executives}")
        non_matching_executives = [exec for exec in budget_executives if exec not in sales_executives]
        if non_matching_executives:
            logger.warning(f"Budget file contains executives not in sales file: {non_matching_executives}")

        # Filter by selected executives (from sales file only)
        filtered_sales_df = filtered_sales_df[filtered_sales_df[sales_exec_col].isin(sales_executives)]
        if filtered_sales_df.empty:
            st.error(f"No sales data found for selected executives: {sales_executives}")
            return None, None, None, None

        # Filter budget data to include only executives from sales_executives
        budget_df = budget_df[budget_df[budget_exec_col].isin(sales_executives)]
        if budget_df.empty:
            st.warning("No budget data found for selected sales executives. Using zero budget values.")

        # Standardize SL Code and Product Group
        ...
        # **Budget vs Billed Value and Quantity (Slides 2 and 3)**

        # Budget: Full amounts by executive (only for sales_executives)
        budget_value_per_exec = budget_df.groupby(budget_exec_col)[budget_value_col].sum().reset_index()
        budget_value_per_exec.columns = ['Executive', 'Budget Value']
        budget_qty_per_exec = budget_df.groupby(budget_exec_col)[budget_qty_col].sum().reset_index()
        budget_qty_per_exec.columns = ['Executive', 'Budget Qty']

        # Get unique SL Code and Product Group pairs from budget for mapping
        budget_pairs = budget_df[[budget_sl_code_col, budget_product_group_col]].drop_duplicates()
        budget_pairs.columns = ['SL Code', 'Product Group']

        # Filter sales to include only matching SL Code and Product Group
        sales_value_mapped = pd.merge(
            filtered_sales_df,
            budget_pairs,
        ...
        if sales_value_mapped.empty:
            st.warning("No sales data matches budget SL Code and Product Group. Using unfiltered sales data.")
            sales_value_mapped = filtered_sales_df.copy()

        # Group sales by executive for mapped data
        sales_grouped_value_mapped = (
            sales_value_mapped.groupby(sales_exec_col)
            .agg({sales_value_col: 'sum'})
            .reset_index()
            .rename(columns={sales_exec_col: 'Executive', sales_value_col: 'Billed Value'})
        )
        sales_grouped_qty_mapped = (
            sales_value_mapped.groupby(sales_exec_col)
        ...
        # Adjust billed value if it exceeds budget
        budget_vs_billed_value_df['Billed Value'] = budget_vs_billed_value_df.apply(
            lambda row: row['Budget Value'] if row['Billed Value'] > row['Budget Value'] and row['Billed Value'] >= 0 else row['Billed Value'],
            axis=1
        )
        ...
        # Ensure only sales executives are in the final DataFrame
        budget_vs_billed_value_df = budget_vs_billed_value_df[
            budget_vs_billed_value_df['Executive'].isin(sales_executives + ['TOTAL'])
        ]
        ...
        # Adjust billed quantity if it exceeds budget
        budget_vs_billed_qty_df['Billed Qty'] = budget_vs_billed_qty_df.apply(
            lambda row: row['Budget Qty'] if row['Billed Qty'] > row['Budget Qty'] and row['Billed Qty'] >= 0 else row['Billed Qty'],
            axis=1
        )
        ...
        budget_vs_billed_qty_df = budget_vs_billed_qty_df[
            budget_vs_billed_qty_df['Executive'].isin(sales_executives + ['TOTAL'])
        ]

        # **Overall Sales Quantity and Value (Slides 4 and 5)**

        # Sales: Use all sales data for selected executives without SL Code/Product Group mapping
        sales_grouped_qty_all = (
            filtered_sales_df.groupby(sales_exec_col)
            .agg({sales_qty_col: 'sum'})
            .reset_index()
            .rename(columns={sales_exec_col: 'Executive', sales_qty_col: 'Billed Qty'})
        )
        sales_grouped_value_all = (
            filtered_sales_df.groupby(sales_exec_col)
            .agg({sales_value_col: 'sum'})
            .reset_index()
            .rename(columns={sales_exec_col: 'Executive', sales_value_col: 'Billed Value'})
        ...
        # Overall Sales Quantity DataFrame (Slide 4, no % column)
        ...
        # Add total row for overall sales quantity
        total_budget_qty = overall_sales_qty_df['Budget Qty'].sum()
        total_billed_qty = overall_sales_qty_df['Billed Qty'].sum()
        total_row_qty = pd.DataFrame({
            'Executive': ['TOTAL'],
            'Budget Qty': [total_budget_qty],
            'Billed Qty': [total_billed_qty]
        })
        overall_sales_qty_df = pd.concat([overall_sales_qty_df, total_row_qty], ignore_index=True)
        ...
        overall_sales_qty_df = overall_sales_qty_df[
            overall_sales_qty_df['Executive'].isin(sales_executives + ['TOTAL'])
        ]

        # Overall Sales Value DataFrame (Slide 5, no % column, 2 decimal places)
        ...
        # Add total row for overall sales value
        total_budget_value = overall_sales_value_df['Budget Value'].sum()
        total_billed_value = overall_sales_value_df['Billed Value'].sum()
        total_row_value = pd.DataFrame({
            'Executive': ['TOTAL'],
            'Budget Value': [total_budget_value],
            'Billed Value': [total_billed_value]
        })
        overall_sales_value_df = pd.concat([overall_sales_value_df, total_row_value], ignore_index=True)
        overall_sales_value_df['Budget Value'] = overall_sales_value_df['Budget Value'].round(2)
        overall_sales_value_df['Billed Value'] = overall_sales_value_df['Billed Value'].round(2)

        # Ensure only sales executives are in the final DataFrame
        overall_sales_value_df = overall_sales_value_df[
            overall_sales_value_df['Executive'].isin(sales_executives + ['TOTAL'])
        ]
...
def create_customer_table(sales_df, date_col, branch_col, customer_id_col, executive_col, selected_months=None, selected_branches=None, selected_executives=None):
    ...
    for fin_year in sorted(available_financial_years):
        ...
        # Get unique executives to display
        ...
        # Create result dataframe with all executives
...
def filter_os_qty(os_df, os_area_col, os_qty_col, os_due_date_col, os_exec_col, 
    ...
    if selected_executives and sorted(selected_executives) != sorted(all_executives):
        ...
        if os_df.empty:
            ...
            return None, None, None
    os_df[os_qty_col] = pd.to_numeric(os_df[os_qty_col], errors='coerce').fillna(0) / 100000  # Divide by 100000
...
# Remove the division by 100000
    final[val_cols] = final[val_cols].div(100000)
    final[val_cols] = final[val_cols].round(2)
    round_cols = ["Overall % Achieved", "% Achieved (Selected Month)"]  # Round percentages
    ...
    with tabs[0]:
        ...
        if not st.session_state.sales_file or not st.session_state.budget_file:
            ...
            try:
                ...
                sales_df = pd.read_excel(st.session_state.sales_file, sheet_name=sales_sheet, header=sales_header_row, dtype={'Customer Code': str})
                ...
                # Sales column auto-mapping
                sales_column_mappings = {
                    'sales_date': ['Date'],
                    'sales_area': ['Branch'],
                    'sales_value': ['Value'],
                    'sales_qty': ['Actual Quantity'],
                    'sales_product_group': ['Type (Make)'],
                    'sales_sl_code': ['Customer Code'],
                    'sales_exec': ['Executive Name']
                }

                # Budget column auto-mapping
                budget_column_mappings = {
                    ...
                    'budget_sl_code': ['SL Code'],
                    'budget_exec': ['Executive Name']
                }

                # Initialize default selections
                default_sales_cols = {}
                for key, targets in sales_column_mappings.items():
                    default_sales_cols[key] = find_column(sales_columns, targets)

                default_budget_cols = {}
                for key, targets in budget_column_mappings.items():
                    default_budget_cols[key] = find_column(budget_columns, targets)

                with st.expander("Column Mappings"):
                    st.subheader("Sales Column Mapping")
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        sales_date_col = st.selectbox(
                            "Sales Date Column",
                            sales_columns,
                            index=sales_columns.index(default_sales_cols['sales_date']) if default_sales_cols['sales_date'] in sales_columns else 0,
                            key='sales_date'
                        )
                        sales_area_col = st.selectbox(
                            "Sales Area Column",
                            sales_columns,
                            index=sales_columns.index(default_sales_cols['sales_area']) if default_sales_cols['sales_area'] in sales_columns else 0,
                            key='sales_area'
                        )
                    with col2:
                        sales_value_col = st.selectbox(
                            "Sales Value Column",
                            sales_columns,
                            index=sales_columns.index(default_sales_cols['sales_value']) if default_sales_cols['sales_value'] in sales_columns else 0,
                            key='sales_value'
                        )
                        sales_qty_col = st.selectbox(
                            "Sales Quantity Column",
                            sales_columns,
                            index=sales_columns.index(default_sales_cols['sales_qty']) if default_sales_cols['sales_qty'] in sales_columns else 0,
                            key='sales_qty'
                        )
                    with col3:
                        sales_product_group_col = st.selectbox(
                            "Sales Product Group Column",
                            sales_columns,
                            index=sales_columns.index(default_sales_cols['sales_product_group']) if default_sales_cols['sales_product_group'] in sales_columns else 0,
                            key='sales_product_group'
                        )
                        sales_sl_code_col = st.selectbox(
                            "Sales SL Code Column",
                            sales_columns,
                            index=sales_columns.index(default_sales_cols['sales_sl_code']) if default_sales_cols['sales_sl_code'] in sales_columns else 0,
                            key='sales_sl_code'
                        )
                    sales_exec_col = st.selectbox(
                        "Sales Executive Column",
                        sales_columns,
                        index=sales_columns.index(default_sales_cols['sales_exec']) if default_sales_cols['sales_exec'] in sales_columns else 0,
                        key='sales_exec'
                    )
                    st.subheader("Budget Column Mapping")
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        budget_area_col = st.selectbox(
                            "Budget Area Column",
                            budget_columns,
                            index=budget_columns.index(default_budget_cols['budget_area']) if default_budget_cols['budget_area'] in budget_columns else 0,
                            key='budget_area'
                        )
                        budget_value_col = st.selectbox(
                            "Budget Value Column",
                            budget_columns,
                            index=budget_columns.index(default_budget_cols['budget_value']) if default_budget_cols['budget_value'] in budget_columns else 0,
                            key='budget_value'
                        )
                    with col2:
                        budget_qty_col = st.selectbox(
                            "Budget Quantity Column",
                            budget_columns,
                            index=budget_columns.index(default_budget_cols['budget_qty']) if default_budget_cols['budget_qty'] in budget_columns else 0,
                            key='budget_qty'
                        )
                        budget_product_group_col = st.selectbox(
                            "Budget Product Group Column",
                            budget_columns,
                            index=budget_columns.index(default_budget_cols['budget_product_group']) if default_budget_cols['budget_product_group'] in budget_columns else 0,
                            key='budget_product_group'
                        )
                    with col3:
                        budget_sl_code_col = st.selectbox(
                            "Budget SL Code Column",
                            budget_columns,
                            index=budget_columns.index(default_budget_cols['budget_sl_code']) if default_budget_cols['budget_sl_code'] in budget_columns else 0,
                            key='budget_sl_code'
                        )
                        budget_exec_col = st.selectbox(
                            "Budget Executive Column",
                            budget_columns,
                            index=budget_columns.index(default_budget_cols['budget_exec']) if default_budget_cols['budget_exec'] in budget_columns else 0,
                            key='budget_exec'
                        )

                st.subheader("Executive Selection")
                sales_executives = sorted(sales_df[sales_exec_col].dropna().astype(str).str.strip().str.upper().unique().tolist())
                sales_executives = [exec for exec in sales_executives if exec != "ACCLP"]
                if 'exec_select_all' not in st.session_state:
                ...
                with st.expander("Column Mappings"):
                    ...
                    with col2:
                        os_feb_net_value_col = st.selectbox(
                            "Net Value Column",
                            os_feb.columns.tolist(),
                            index=os_feb.columns.tolist().index(default_os_feb_cols['net_value']) if 'net_value' in default_os_feb_cols else 0,
            ...
            # Get available months dynamically
                try:
                   ...
                   available_months = sorted(sales_df[date_col].dt.strftime('%b %Y').dropna().unique().tolist())
                    ...
                    available_months = []
                # Month filter below column mapping
                ...
                with filter_tab1:
                   raw_branches = sales_df[branch_col].dropna().astype(str).unique().tolist()
